...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    if (user) {
      const getCartData = async () => {
        ...
        setIsInitialLoading(false); // Set loading to false after fetching cart data
      };
      getCartData();
    }
  }, [user]);
  ...
  useEffect(() => {
    if (!isAddressLoading && cartData.length > 0) {
      setIsInitialLoading(false); // Set loading to false after fetching address data and cart data
    }
  }, [isAddressLoading, cartData]);
  ...
  return (
    <RootLayout>
      <div>
        {isInitialLoading || isAddressLoading ? ( // Render "Loading" if any data is still loading
          <div className="flex flex-col justify-center items-center min-h-[50vh]">
            <Loader />
          </div>
        ) : (
          <>
            {/* Render your checkout page content here */}
                  ...
                  <div className="relative">
                    <ul className="relative flex w-full items-center justify-between space-x-2 sm:space-x-4">
                      {steps.map((step, index) => (
                        ...
                        >
                          <a
                            className={`flex h-6 w-6 items-center justify-center rounded-full ${index === currentStep
                          ...
                          <span
                            className={`font-semibold ${index === currentStep
                              ? "text-gray-900"
                              : "text-gray-500"
                              }`}
                          >
                        ...
                        <ul className="my-8 md::w-[60%] flex flex-col gap-4">
                          {cartData &&
                            cartData?.map((data) => {
                              ...
                              const itemPrice = book
                                ? calculateItemPrice(book.price, quantity)
                                : 0;
                              return (
                  ...
                  <>
                    <div className="px-4 pt-8 w-full">
                      ...
                      <div className="flex flex-col gap-4 border p-2 rounded">
                        {AddressData &&
                          AddressData.map((addressValueData, index) => (
                    ...
                    {showAlert && (
                      <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-75">
                        <div className="bg-white p-4 rounded shadow-md">
                          ...
                          <button
                            className="mt-4 rounded-md bg-gray-900 px-6 py-3 font-medium text-white"
                            onClick={closeAlert}
                          >
                            Close
                          </button>
                ...
                >
                  Place Order
                    ...
                    <div className="mt-10 bg-gray-50 px-4 pt-8 lg:mt-0">
                      ...
                      <button
                        onClick={handelOrderNow}
                        className={`mt-4 mb-8 text-black rounded-md border px-6 py-3 font-medium ${!paymentMethod
                          ? "bg-gray-300 cursor-not-allowed"
                          : "bg-green-500"
                          }`}
                        disabled={!paymentMethod}
