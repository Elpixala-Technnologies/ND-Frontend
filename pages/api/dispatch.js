...
export default async function handler(req, res) {
  const { name, email, filterdOrder, orderTrackingLink, totalPrice } =
    req.body;
  ...
  const mailOptions = {
    ...
    to: email,
    subject: "Your Order Dispatch Successfully Done + Thank you for your order " + name,
    text: `Name: ${name}\nEmail: ${email}`,
    html: `
        <ul  style="padding: 10px;">
            ${filterdOrder?.book?.map((product) => `
                <li>
                    ...
                    <p><a href="${orderTrackingLink}">Track your order here</a></p>
